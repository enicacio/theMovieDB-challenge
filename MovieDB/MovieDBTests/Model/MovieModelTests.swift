...
final class MovieModelTests: XCTestCase {
    ...
    // MARK: - Genre Decoding Tests
    func testDecodeMovieWithGenreIds() async throws {
        // Movie de /movie/popular com genre_ids
        let json = """
        {
          "id": 550,
          "title": "Fight Club",
    ...
    func testDecodeMovieWithGenresArray() async throws {
        // Movie de /movie/{id}?append_to_response=genres
        let json = """
        {
          "id": 550,
          "title": "Fight Club",
        ...
        // Deve extrair IDs dos genres
        XCTAssertEqual(movie.genreIds, [18, 28, 53])
    }

    func testDecodeMovieWithBothGenreIdAndGenres() async throws {
        // Se vir ambos, genre_ids tem prioridade
        let json = """
        {
          "id": 550,
          "title": "Fight Club",
        ...
        XCTAssertEqual(movie.genreIds, [18])  // genre_ids tem prioridade
    }
    ...
    func testEncodeMoviePreservesAllFields() throws {
        ...
        let decoded = try JSONDecoder().decode(Movie.self, from: encoded)
        
        XCTAssertEqual(decoded.voteCount, 29696)
        XCTAssertEqual(decoded.runtime, 139)
        XCTAssertEqual(decoded.tagline, "Your mind...")
